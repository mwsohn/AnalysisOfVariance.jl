...
## Examples
We will use `auto` dataset downloaded from https://vincentarelbundock.github.io/Rdatasets/csv/causaldata/auto.csv is used in this example. We want to
...
function Base.show(io::IO, a::ANOVA)
    n = length(a.ss)
    ...
    pretty_table(io, 
            hcat(
                ...
                a.ms,
                a.F,
                vcat(pstr, missing, missing)),
        column_labels = ["Source", "SS","DF","MS","F","P"],
        formatters=[(v,i,j) -> ismissing(v) ? "" : v, fmt__printf("%.3f", [2, 4, 5])],
        table_format=TextTableFormat(;
            @text__no_vertical_lines,
            horizontal_lines_at_column_labels=[1],
            horizontal_line_at_data_row=[length(a.title) -1]
